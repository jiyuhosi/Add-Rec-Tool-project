...
| companyNameKana                      | String  | 企業名（カナ）     | 必須 | テキストエリア                | 会社名のフリガナ       | カタカナ・漢字・ひらがなのみ/最大 255 文字                                    | 255    |
...
| phoneNumber                          | String  | 電話番号           | 任意 | テキストエリア                | 電話番号               | 数字のみ(ハイフン無し)推奨/最大 255 文字                                      | 255    |
| postalCode                           | String  | 郵便番号           | 必須 | テキスト/入力後に住所自動入力 | 郵便番号               | 数字のみ(ハイフン無し)/入力後 API による住所補完                              | 255    |
| prefecture                           | String  | 都道府県           | 必須 | プルダウン                    | 都道府県名             | JIS コード/選択式                                                             | -      |
| city                                 | String  | 市区町村           | 必須 | テキストエリア                | 市区町村名             | 自動入力可/最大 255 文字                                                      | 255    |
| streetAddress                        | String  | 番地               | 必須 | テキストエリア                | 番地                   | 数字・全角ハイフン/最大 255 文字                                              | 255    |
| addressLine                          | String  | 建物部屋番号       | 任意 | テキストエリア                | 建物名、部屋番号など   | カタカナ・漢字・ひらがな・大文字英・全角ハイフン/最大 255 文字                | 255    |
...
| ownerLoginPassword                   | String  | パスワード         | 必須 | テキストエリア                | ログイン用パスワード   | 6-12 文字/英大小・数字各 1 種以上必須/半角のみ/記号任意/過去 5 回の再利用不可 | 12     |
...
| occupationalDoctorIntegrationEnabled | Boolean | 産業医連携         | 必須 | ラジオ(なし)                  | 産業医連携の有無       | UI 上は選択肢提示可だが現在は "なし" 固定                                     | -      |
...
-   companyNameKana: `/^[\p{Script=Hiragana}\p{Script=Katakana}\p{Script=Han}ー]+$/u`、max 255
...
-   safetyConfirmationEnabled, employeeChatEnabled は appIntegrationEnabled が true のときのみ表示。
-   occupationalDoctorIntegrationEnabled は仕様上 "なし" 固定。将来の拡張前提で UI は残置可。
